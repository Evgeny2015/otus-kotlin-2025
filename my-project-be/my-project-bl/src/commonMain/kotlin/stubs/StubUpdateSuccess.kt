...
fun ICorChainDsl<DevContext>.stubUpdateSuccess(title: String, corSettings: DevCorSettings) = worker {
    this.title = title
    this.description = """
        Кейс успеха для изменения устройства
    """.trimIndent()
    ...
    handle {
        logger.doWithLogging(id = this.requestId.asString(), LogLevel.DEBUG) {
            ...
            val stub = DevStub.prepareResult {
                devRequest.id.takeIf { it != DevId.NONE }?.also { this.id = it }
